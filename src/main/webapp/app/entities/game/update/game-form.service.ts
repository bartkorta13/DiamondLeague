 ...
 */
type FormValueOf<T extends IGame | NewGame> = Omit<T, 'date'> & {
  date?: string | null;
};
...
type GameFormDefaults = Pick<NewGame, 'id' | 'date'>;

type GameFormGroupContent = {
  id: FormControl<GameFormRawValue['id'] | NewGame['id']>;
  date: FormControl<GameFormRawValue['date']>;
  stadium: FormControl<GameFormRawValue['stadium']>;
};
...
export class GameFormService {
  createGameFormGroup(game: GameFormGroupInput = { id: null }): GameFormGroup {
    ...
    return new FormGroup<GameFormGroupContent>({
      ...
      date: new FormControl(gameRawValue.date, {
        validators: [Validators.required],
      }),
      stadium: new FormControl(gameRawValue.stadium, {
        validators: [Validators.required],
      }),
    });
  }
  ...
  private convertGameRawValueToGame(rawGame: GameFormRawValue | NewGameFormRawValue): IGame | NewGame {
    return {
      ...rawGame,
      date: dayjs(rawGame.date, DATE_TIME_FORMAT),
    };
  }
  ...
  private convertGameToGameRawValue(
    ...
    return {
      ...game,
      date: game.date ? game.date.format(DATE_TIME_FORMAT) : undefined,
    };
  }
}
